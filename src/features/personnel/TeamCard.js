...
function TeamCard({ team, handleOpenFromTeam, handleOpenRemoveTeams }) {
  const { _id: targetTeamId, manager, workers, name } = team;

  return (
    <Card sx={{ display: "flex", alignItems: "center", p: 1, position: "relative" }}>
      <Box sx={{ padding: "10px" }}>
        <Box sx={{}}>
          ...
          >
            ...
            <Button onClick={() => handleOpenFromTeam(targetTeamId)}>
        ...
        <Box>
          ...
          >
            ...
            <Typography sx={{ color: "text.secondary", ml: "10px", mt: "auto" }}>{manager.name}</Typography>
          </Box>
        </Box>
        <Box>
          <Typography variant="body2" sx={{ fontWeight: 600, color: "#1f1f1f", fontSize: "1rem", margin: "22px 0  6px" }}>
          ...
          {workers.length === 0 ? (
            <Box sx={{ height: 22, m: 1 }}></Box>
          ) : (
            <AvatarGroup sx={{ justifyContent: "flex-end", ml: 2, mt: 1 }} max={4}>
              {workers.map((worker, index) => {
                return <Avatar key={index} alt={worker.name} src={worker.avatarUrl} sx={{ width: 26, height: 26 }} />;
              })}
            </AvatarGroup>
          )}
        </Box>
