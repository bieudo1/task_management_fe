...
const slice = createSlice({
  ...
  reducers: {
    ...
    getTeam(state, action) {
      ...
      state.usersInTeam = team.workers.map((user) => ({ value: user._id, label: user.name, avatar: user?.avatarUrl }));
    },
...
export const newProject =
  ({ name, teamId, status, description, assignee }) =>
  async (dispatch) => {
    dispatch(slice.actions.startLoading());
