...
import { Box, Card, Container, Grid, Modal, List, Avatar, Button, Typography } from "@mui/material";
...
function ProjectProfilePage() {
  ...
  const { singleProject, progress, isLoading } = useSelector((state) => state.project, shallowEqual);
  ...
  useEffect(() => {
    if (statusArchive && totalTasks !== 0) {
      if (statusArchive.length === totalTasks) {
        setFinished(true);
      }
    }
  ...
  return (
    <Container sx={{ margin: "64px" }}>
              ...
              <Grid item xs={12} md={3}>
                <Card>
                  <Box sx={{ margin: "10px", display: "grid" }}>
                    ...
                    <Typography variant="h7" sx={{ color: "text.secondary" }}>
                      Created by:
                      <Avatar alt={singleProject.assigner.name} src={singleProject.assigner.imageUrl} sx={{ width: 40, height: 40 }} />
                      {singleProject.assigner.name}
                    </Typography>
                    <Box>
                ...
                {finished && user.position === "Manager" && (
                  <Button variant="contained" size="small" sx={{ margin: "10px" }} onClick={() => handleOpenFinished()}>
                    Project Finished
                  </Button>
                )}
              </Grid>
      ...
      <Modal open={openFinished} onClose={handleCloseFinished} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <Box sx={style}>
          <Typography>bạn có muốn kết thúc không ?</Typography>
          <Box>
            <Button sx={{ color: "red" }} onClick={() => handleProjectFinished(singleProject._id)}>
              Yes
            </Button>

            <Button sx={{ color: "green" }} onClick={() => handleCloseFinished()}>
              No
            </Button>
          </Box>
        </Box>
