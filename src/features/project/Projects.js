...
import { Box, Modal, Container, Stack, Typography, Button, Grid, TablePagination, Card } from "@mui/material";
...
function Projects() {
  ...
  const { currentPageProjects, projectsById, isLoading, count } = useSelector((state) => state.project);
    ...
    <Container sx={{ margin: "64px" }}>
      <>
        {user.position === "Manager" && (
          <Button sx={{ m: "10px" }} onClick={() => handleOpenNewProject()}>
            New Project
          </Button>
        )}
        <Modal open={openNewProject} onClose={handleCloseEditProject} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
          <Box sx={styleModal}>
            <Stack spacing={3}>
              <EditProject handleCloseEditProject={handleCloseEditProject} />
            </Stack>
          </Box>
        </Modal>
      </>
      {isLoading || !listProjects ? (
        ...
        <Container>
          ...
          <Typography variant="subtitle" sx={{ color: "text.secondary", ml: 1 }}>
            {count > 1 ? `${count} users found` : count === 1 ? `${count} user found` : "No user found"}
          </Typography>
          ...
          <Grid container spacing={3} my={1}>
            {listProjects.map((project) => (
              <Grid key={project._id} item xs={12} md={4}>
                <ProjectCard project={project} handleOpenRemoveProjects={handleOpenRemoveProjects} handleOpenEditProject={handleOpenEditProject} />
              </Grid>
            ))}
          </Grid>
        </Container>
        ...
        <div>
          <Box sx={styleModal}>
            <Card sx={centered}>
              ...
              <Box>
                <Button sx={{ color: "red" }} onClick={() => handleRemoveProject(projectId)}>
                  Yes
                </Button>

                <Button sx={{ color: "green" }} onClick={() => handleCloseRemoveProjects()}>
                  No
                </Button>
              </Box>
            </Card>
      ...
      <Modal open={openEditProject} onClose={handleCloseEditProject} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <Box sx={styleModal}>
          <EditProject handleCloseEditProject={handleCloseEditProject} projectId={projectId} />
        </Box>
      </Modal>
    </Container>
  );
