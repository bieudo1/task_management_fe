...
import { Stack, Container, Typography, Modal, List, Button, Box, Card } from "@mui/material";
...
import { centered, style, styleModal } from "../../app/config";
  ...
  return (
    <Container>
      <Box>
        ...
        <Modal open={allTask} onClose={handleCloseAllTasks} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
          <Box sx={styleModal}>
            <Stack spacing={3}>
      ...
      {tasks.length === 0 ? (
        ...
        <>
          ...
          <Modal open={openRemoveTask} onClose={handleCloseRemoveTask} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
            <Box sx={styleModal}>
              <Card sx={centered}>
                <Typography>bạn có muốn xóa không ?</Typography>
                <Box>
                  <Button sx={{ color: "red" }} onClick={() => handleRemoveTask(taskId)}>
                    Yes
                  </Button>

                  <Button sx={{ color: "green" }} onClick={() => handleCloseRemoveTask()}>
                    No
                  </Button>
                </Box>
              </Card>
          ...
          <Modal open={openEditTask} onClose={handleCloseEditTask} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
            <Box sx={styleModal}>
              <EditTask handleCloseEditTask={handleCloseEditTask} projectId={projectId} assignee={assignee} taskId={taskId} />
            </Box>
          </Modal>
        </>
      )}
